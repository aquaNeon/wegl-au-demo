...
(function() {
  ...
  function init() {
    ...
    for (let i = 0; i < NODE_COUNT; i++) {
      ...
      const radius2 = CONFIG.sphereRadiusEnd * (0.2 + Math.random() * 0.65);
    ...
    /* ── Scroll — REMOVED, progress locked at 1.0 ── */

    // ↓↓ CHANGE 1: progress is permanently 1.0, no scroll listener
    let scrollProgress = 1.0, targetProgress = 1.0;

    /* ── Render Loop ── */

    const clock = new THREE.Clock();
    let bgT = 1; // start at end-state bg

    function animate() {
      requestAnimationFrame(animate);
      const time = clock.getElapsedTime();

      // ↓↓ CHANGE 2: p is always 1.0, auto-rotate instead of scroll-driven rotation
      const p = 1.0;
      drag.rotationY += 0.0000;
      ...
      const quat = new THREE.Quaternion().setFromEuler(
        new THREE.Euler(0, drag.rotationY, 0, 'YXZ')
      );
      ...
      const mouseGate = 1.0; // always active since p=1
      if (mouse.isOver && !drag.active) {
      ...
      blueMat.uniforms.uVisibility.value     = 1.0;
      if (isDark) {
